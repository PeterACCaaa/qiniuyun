...
export function buildPullRequestReport(snapshot) {
	...
	return {
		...
		fileContexts: [],
		nextSteps: [
			"用 AI 结合 PR 上下文解释最高风险项。",
			"为需要更多上下文的风险项补全相关源码。",
			"生成 reviewer 可直接使用的 Markdown 评论。",
		],
		markdown,
	};
}
...
function buildSummary(snapshot, fileNames) {
	if (!fileNames.length) {
		return "已拉取真实 GitHub PR 数据，但 GitHub 没有返回变更文件。";
	}

	const preview = fileNames.slice(0, 5).join(", ");
	const suffix = fileNames.length > 5 ? ", ..." : "";
	return `已拉取真实 GitHub PR 数据。本次 PR 修改 ${snapshot.pr.changedFiles} 个文件，新增 ${snapshot.pr.additions} 行，删除 ${snapshot.pr.deletions} 行：${preview}${suffix}。`;
}

function buildRiskOverview(findings, counts) {
	if (!findings.length) {
		return "规则扫描没有发现明显确定性风险。下一步 AI 分析可以继续检查语义层面的代码问题。";
	}

	return `风险地图发现 ${findings.length} 个有证据支撑的审查重点：${counts.blocking} 个阻塞、${counts.warning} 个警告、${counts.suggestion} 个建议。每条结果都包含证据、影响和复核方式。`;
}
...
function buildMarkdown(snapshot, findings) {
	const lines = [
		"## PR 概览",
		"",
		`- 标题：${snapshot.pr.title}`,
		`- 作者：${snapshot.pr.author}`,
		`- 分支：${snapshot.pr.headBranch} -> ${snapshot.pr.baseBranch}`,
		`- 变更文件：${snapshot.pr.changedFiles}`,
		`- 行数变化：+${snapshot.pr.additions} / -${snapshot.pr.deletions}`,
		"",
		"### 变更文件",
		...snapshot.changedFiles.map(
			(file) =>
				`- ${displayFileStatus(file.status)} ${file.filename} (+${file.additions} / -${file.deletions})`,
		),
	];
	...
	if (findings.length) {
		...
		for (const finding of findings) {
			lines.push(
				`- [${displaySeverity(finding.severity)}/${displayCategory(finding.category)}] ${finding.file}:${finding.lineHint} ${displayTitle(finding.title)}`,
				`  - 证据：${translateEvidence(finding.evidence)}`,
				`  - 影响：${impactText[finding.impact] || finding.impact}`,
				`  - 建议：${suggestionText[finding.suggestion] || finding.suggestion}`,
				`  - 复核方式：${verifyText[finding.howToVerify] || finding.howToVerify}`,
			);
		}
	}
	...
	lines.push(
		"",
		"### 上下文补全",
		"- 高风险文件会在 AI Review 前补全 base/head 两侧的完整文件上下文。",
	);
...
function displaySeverity(value) {
	return severityText[value] || value;
}
...
function displayFileStatus(value) {
	return fileStatusText[value] || value;
}
...
function translateEvidence(value) {
	let result = value
		...
		.replace(
			/^(\d+) files changed, (\d+) total changed lines$/,
			"$1 个文件发生变更，总计 $2 行变化",
		)
		.replace(
			/^(\d+) changed lines in one file$/,
			"单文件发生 $1 行变更",
		)
		.replace(
			/^file status is removed and path matches test patterns$/,
			"文件被删除，且路径匹配测试文件模式",
		);
